...
# ~6min, 10+min... (distracted, solved as recursive?), ~5min?, 7min (stupid bugs / burnt out)
...
# ~7min, 4min, 1.25min, 10+min, 2.5min(after walk / break)
...
class Array
  ...
  def dups
    ...
    (0...self.length).each { |idx| hash[self[idx]] << idx }
    hash.select { |_k,v| v.length > 1 }
  end
end
...
# ~12min (dumb error, not recursing on left and right assignment)
# 4min, no errors first run
